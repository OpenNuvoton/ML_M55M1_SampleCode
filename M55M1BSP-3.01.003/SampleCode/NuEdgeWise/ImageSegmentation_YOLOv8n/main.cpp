 ...
 * @file     main.cpp
 * @version  V1.00
 * @brief    YOLOv8n segmentation network sample. Demonstrate image segmentaion.
 *
...
#include "YOLOv8nSegModel.hpp"       /* Model API */
#include "YOLOv8nSegPostProc.hpp"       /* Post processing API */
#include "Labels.hpp"
...
#define ACTIVATION_BUF_SZ (1458176)
...
{
    float cbrt_label = std::cbrt(numLables + 1);
    uint32_t R = 0;
    uint32_t G = 0;
    uint32_t B = 0;
...
#define MODEL_FILE "0:\\YOLOv8nSeg_192_vela.tflite"
#else
#define MODEL_FILE "1:\\YOLOv8nSeg_192_vela.tflite"
#endif
...
int main()
{
    ...
    printf("Please copy ImageSegmentation_YOLOv8n/Model/YOLOv8nSeg_192_vela.tflite to SDCard:/YOLOv8nSeg_192_vela.tflite \n");
    printf("===================================================================================\n");
    ...
    // postProcess
    arm::app::yolov8n_seg::YOLOv8nSegPostProcessing postProc(&model, 0.5);
    image_t segImg;
...
#define EACH_PERF_SEC 5
    ...
    while (1)
    {
        ...
        if (infFramebuf)
        {
            ...
            imlib_nvt_RGB_blend(&segResizeImg, &infFramebuf->frameImage, &infFramebuf->frameImage, 0.4);
#if defined(__PROFILE__)
            u64EndCycle = pmu_get_systick_Count();

...
extern unsigned char *GetModelBinAddress();

int main()
{
	char *psModelBinAddr = (char *)GetModelBinAddress();

    /* Initialise the UART module to allow printf related functions (if using retarget) */
    BoardInit();

	printf("Model bin location is %p \n", psModelBinAddr);
	
	//JSON parser
	if(ModelBinParser_Init(psModelBinAddr) != 0)
	{
        printf_err("Unable load model/label binary data\n");
        return 1;
	}

	int i32TfliteOffset = ModelBinParser_GetTfliteOffset(); 
	int i32TfliteSize = ModelBinParser_GetTfliteSize();

	if((i32TfliteOffset < 0) || (i32TfliteSize) < 0)
	{
        printf_err(" model location incorrect \n");
        return 1;
	}
	
	char *psTfliteAddr = psModelBinAddr + i32TfliteOffset;

    //label information
    std::vector <std::string> labels;
    ModelBinParser_GetLabels(labels);
	
	info("the label total item is %d \n", labels.size());

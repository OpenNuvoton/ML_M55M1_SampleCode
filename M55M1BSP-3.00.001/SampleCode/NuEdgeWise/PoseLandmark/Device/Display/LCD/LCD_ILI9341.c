...
static void ili9341_send_pixels(uint16_t *pixels, uint32_t fixed_color, int32_t byte_len)
{
...
void ili9341_put_char8x16(uint16_t x, uint16_t y, uint8_t c, uint32_t fColor, uint32_t bColor)
{
    uint32_t i, j;
    uint8_t m;

    ili9341_set_column(x, x + 8 - 1);
    ili9341_set_page(y, y + 16 - 1);
